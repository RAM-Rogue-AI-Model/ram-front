...
const Button = (props:ButtonType) => {
    ...
    return (
        <div className={classnames} onClick={(e)=>onButtonClicked(e)} onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}>
            {props.label && <Body underline={props.type === "link"} hover={(hover && props.type !== "primary") ? "secondary" : null} primary={props.type === "link"} hoverable={props.type === "link"} auto>{t(props.label)}</Body>}
