...
const AttackPopup = (props: AttackPopupType) => {
  const onEnemyClicked = (enemy: EnemyType) => {
    if (props.onEnemyClicked)
      props.onEnemyClicked({
        type: 'attack',
        target_id: enemy.id,
      });
  };
    ...
    <div className="ActionPopup">
      ...
      >
        <div className="ActionPopupContent">
          {props.ennemies &&
            props.ennemies.length > 0 &&
            props.ennemies.map((enemy: EnemyType, idEnemy: number) => (
              <GameButton
                key={'key-enemy-choice-' + idEnemy}
                label={enemy.name}
                onClick={() => {
      ...
      >
        <div className="ActionPopupContent">
          {props.items && props.items.length > 0 ? (
            props.items.map((item: ItemType, idItem: number) => (
              <GameButton
                key={'key-item-choice-' + idItem}
                label={item.name}
                onClick={() => {
                  onItemClicked(item);
                }}
      ...
      >
        <div className="ViewInventoryContent">
          {props.inventory.map((item: ItemType, itemId: number) => (
            <div
              key={'popup-inventory-' + itemId}
              className="ViewInventoryCase"
            >
              <Body>
                {t('shop.inventory.item', {
                  name: item.name,
  ...
  return (
    <div className="RecapContent">
      {props.steps.map((step: GameStep, stepId: number) => (
        <div
          className={'RecapStep' + (step.completed ? ' completed' : '')}
          key={'step-' + stepId}
        >
          <Body>{stepId + 1}</Body>
...
const Game = () => {
  ...
  const fetchGame = (id: string, playerId: string) => {
    get(`/api/game/${id}?playerId=${playerId}`)
      .then((res) => {
        console.log(res);
        if (res) {
  ...
  const fetchChoices = (id: string, playerId: string) => {
    get(`/api/game/${id}/choices?playerId=${playerId}`)
      .then((res) => {
        if (res) {
  ...
  const createDungeon = (gameId: string, playerId: string) => {
    post(`/api/battle/`, { game_id: gameId, player_id: playerId })
      .then((res) => {
  ...
  useEffect(() => {
    if (game && player) {
      ...
      if (game && game.steps) {
        if (
          game.steps.length === 0 ||
          game.steps[game.steps.length - 1]?.completed
        )
          setStep('CHOICE');
        else {
          const currentDungeon: DungeonType =
            game.steps[game.steps.length - 1].type;
  ...
  useEffect(() => {
    if (nextDungeon && game?.id && player?.id && step !== 'DUNGEON') {
      if (
        ...
        put(`/api/game/${game.id}/dungeon?playerId=${player.id}`, {
          type: nextDungeon,
        })
          .then((res) => {
  ...
  const leaveGame = (popup = true) => {
    if (game?.id && player?.id) {
      del(`/api/game/${game.id}?playerId=${player.id}&force=true`)
        .then(() => {
          navigate('/');
          if (popup) closePopup();
        })
  ...
  const EndGame = (gameId: string, playerId: string) => {
    del(
      `/api/game/${gameId}?playerId=${playerId}&pv=${pv}&attack=${atk}&speed=${spe}`
    )
      .then(() => {
        navigate('/');
      })
  ...
  const completeDungeon = (
    ...
    patch(`/api/game/${gameId}`, { player_id: playerId, item_id: itemId })
      .then((res) => {
  ...
  const battleAction = (battleId: string, action: Action) => {
    post(`/api/battle/${battleId}/action`, { ...action })
      .then((res) => {
        if (res) {
    ...
    if (enemies && enemies.length > 0) {
      openPopup(
        <AttackPopup
          ennemies={enemies}
          closePopup={closePopup}
          onEnemyClicked={(action: Action) => {
            closePopup();
            battleAction(battleId, action);
          }}
        />,
        false
      );
  ...
  const handleInventoryClick = (inventory: ItemType[]) => {
    openPopup(
      <ViewInventory closePopup={closePopup} inventory={inventory} />,
      false
    );
  };
  ...
  const getFighterName = (fighterId: string) => {
    ...
    if (fighterId === 'player') return player.name + ' (joueur)';
      ...
      const id = fighterId.includes('enemy_')
        ? fighterId.split('_')[1]
        : fighterId;
        ...
        {battle.winner ? (
          battle.winner === 'player' ? (
            <div className="BattleContainer">
              ...
              <Body>
                {t('battle.win.gain', {
                  count: 20 * (1 + Math.floor(game.steps.length / 10)),
                })}
              </Body>
              <GameButton
                label="battle.win.action"
                onClick={() => completeDungeon(game.id, player.id)}
              />
            </div>
          ) : (
            <div className="BattleContainer">
              ...
              <div className="BattleContent">
                <Body>
                  {t('battle.defeate.level', {
                    level: game.steps ? game.steps.length - 1 : 0,
                  })}
                </Body>
              </div>
              {game.steps && game.steps.length > 1 && (
                <div className="BattleContent">
              ...
              <div className="BattleContent">
                <Button
                  disabled={
                    game.steps && game.steps.length - 1 - (pv + atk + spe) !== 0
                  }
                  size={'large'}
                  label={'battle.defeate.action'}
                  onClick={() => EndGame(game.id, player.id)}
                />
              </div>
            </div>
          )
          ...
          <div className="BattleContainer">
            <div className="BattleContent">
              <Body>
                {t('battle.turn.title', {
                  count: Object.keys(battle.actions).length + 1,
                })}
              </Body>
            </div>
            <div className="BattleContent">
              ...
              <div className="BattleWrap">
                {battle.enemy &&
                  battle.enemy.length > 0 &&
                  battle.enemy.map((enemy: EnemyType, idEnemy) => (
            ...
            <div className="BattleContent">
              ...
              <div className="GameChoices">
                <GameButton
                  label={t('battle.actions.attack')}
                  onClick={() => {
                    handleAttackClicked(battle.id, battle.enemy);
                  }}
                />
                <GameButton
                  label={t('battle.actions.defend')}
                  onClick={() => {
                    battleAction(battle.id, {
                      type: 'defend',
                      target_id: 'player',
                    });
                  }}
                />
                <GameButton
                  label={t('battle.actions.item')}
                  disabled={!game.consumables || game.consumables.length === 0}
                  onClick={() => {
                    handleItemsClicked(
                      player.id,
                      battle.id,
            ...
            {battle.effect && battle.effect.length > 0 && (
              <div className="BattleContent">
                ...
                <div className="BattleWrap">
                  {battle.effect.map((effect: EffectType, id: number) => (
                    <div key={'effect-' + id} className="EffectCard">
                      <Body>
                        {t('battle.effects.effect.' + effect.stat_name, {
                          count: effect.count,
              ...
              <div className="HistoryContainer">
                ...
                <div className="HistoryContent">
                  {Object.keys(battle.actions).length > 0 ? (
                    Object.values(battle.actions)
                      .reverse()
                      .map((actions: ActionsType, actionId: number) => (
                        <div
                        ...
                        >
                          <div className="HistoryCase fix">
                            <Body>
                              {Object.values(battle.actions).length - actionId}
                            </Body>
                          </div>
                          <div className="HistoryCase col">
                            {Object.keys(actions).map(
                              (fighter: string, idFighter: number) => (
                                <div
                                  className="HistoryDetails"
                                  key={
                                    'turn-' + actionId + '-fighter-' + idFighter
                                  }
                                >
                                  <Body>
                                    {t(
                                      'battle.history.table.actions.' +
                                        actions[fighter].type,
                                      {
                                        name: getFighterName(fighter),
                                        name1: getFighterName(fighter),
                                        name2: getFighterName(
                                          actions[fighter].target_id
                                        ),
                                      }
                                    )}
                                  </Body>
  ...
  const renderShop = () => {
    if (!shop || shop.length === 0 || !game?.id || !player?.id)
      return <Loader />;
      ...
      <div className="GameMap">
        ...
        <div className="GameChoices">
          ...
          {shop.map((item: ItemType, id: number) => (
            <GameButton
              key={'item-' + id}
              label={t('shop.items.item', {
                name: item.name,
                description: item.description,
                price: item.price,
              })}
              onClick={() => {
                completeDungeon(game.id, player.id, item.id);
              }}
            />
          ))}
          <GameButton
            label="shop.items.none"
            onClick={() => {
              completeDungeon(game.id, player.id);
            }}
          />
        </div>
      </div>
  ...
  const renderDataCenter = () => {
    if (!game?.id || !player?.id) return <Loader />;

    return (
      <div className="GameMap">
        <Heading size={'s'}>{t('datacenter.title')}</Heading>
        <Body>{t('datacenter.description')}</Body>
        <GameButton
          label="datacenter.action"
          onClick={() => {
            completeDungeon(game.id, player.id);
          }}
        />
              ...
              <div className="HeaderInfoPart">
                <div className="StatsContent">
                  ...
                  <div className="StatText">
                    <Body size={'large'}>
                      {t('game.stat.pv', {
                        current: battle ? battle.pv : game.pv,
                        total: player.pv,
                      })}
                    </Body>
                  </div>
                  ...
                  <div className="StatDetails">
                    ...
                    {game.consumables?.length === 0 ? (
                      ...
                      <div className="InventoryContent">
                        <Body>
                          {t('game.inventory.count', {
                            count: game.consumables?.length ?? 0,
                          })}
                        </Body>
                        <Button
                          size={'auto'}
                          type={'secondary'}
                          label={'game.inventory.see'}
                          onClick={() =>
                            handleInventoryClick(game.consumables ?? [])
                          }
                        />
                      </div>
                    )}
